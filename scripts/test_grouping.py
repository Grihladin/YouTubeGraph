...
def test_single_video(video_id: str, verbose: bool = True):
    ...
    try:
        ...
        for i, group in enumerate(groups[:3]):
            ...
            print(f"┃")
            print(f"┃ ⏱️  Timeline:")
            print(f"┃    Start:    {group.start_time:>7.1f}s  ({group.start_time/60:>6.2f} min)")
            print(f"┃    End:      {group.end_time:>7.1f}s  ({group.end_time/60:>6.2f} min)")
            print(f"┃    Duration: {group.end_time - group.start_time:>7.1f}s  ({(group.end_time - group.start_time)/60:>6.2f} min)")
            print(f"┃")
            print(f"┃ 📊 Statistics:")
            ...
            if verbose:
                ...
                for seg in group.segments:
                    print(f"┃    • Segment {seg.index}: {seg.start_time:.1f}s → {seg.end_time:.1f}s "
        ...
        if len(groups) > 3:
            print(f"... and {len(groups) - 3} more groups (see {output_path} for full details)\n")
        ...
        print(f"📊 Word Count:")
        print(f"   • In target range (400-800): {in_range}/{len(groups)} ({in_range/len(groups)*100:.0f}%)")
