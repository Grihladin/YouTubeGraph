...
def main():
    ...
    for group in groups_data['groups']:
        print(f"  Group {group['group_id']}: {group['num_segments']} segments, {group['total_words']} words")
    ...
    try:
        ...
        for ec in extracted_list:
            print(f"  Group {ec.group_id}: {len(ec.concepts)} concepts, group_text length: {len(ec.group_text)}")
        ...
        from src.services.relationships.relationship_extractor import RelationshipExtractor
        
        openai_client = OpenAI(api_key=config.openai.api_key, base_url=config.openai.base_url)
