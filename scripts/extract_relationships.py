...
def analyze_concept_relationships(uploader: ConceptUploader, video_id: str):
    ...
    if type_pairs:
        ...
        for i, (pair, count) in enumerate(sorted(type_pairs.items(), key=lambda x: x[1], reverse=True)[:10], 1):
    ...
    for concept in concepts[:10]:  # Limit to first 10 to avoid too many API calls
        similar = uploader.search_concepts(
            concept['name'],
            limit=5,
            min_confidence=0.7
        )
    ...
    print(f"  • Total: {len(cooccurrences) + len(temporal_rels) + len(similarity_rels)}")
...
def main():
    ...
    if not args:
        ...
        print("   Usage: python scripts/extract_relationships.py VIDEO_ID [VIDEO_ID2 ...]")
        sys.exit(1)
