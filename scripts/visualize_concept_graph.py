...
def visualize_video_concepts(uploader: ConceptUploader, video_id: str):
    ...
    print(f"   Avg importance: {sum(c['importance'] for c in concepts) / len(concepts):.2f}")
    print(f"   Avg confidence: {sum(c['confidence'] for c in concepts) / len(concepts):.2f}")
    ...
    for i, concept in enumerate(top_concepts, 1):
        ...
        print(f"    Importance: {concept['importance']:.2f} | Confidence: {concept['confidence']:.2f}")
        print(f"    Time: {concept['firstMentionTime']:.1f}s - {concept['lastMentionTime']:.1f}s (Group {concept['groupId']})")
...
def visualize_all_videos(uploader: ConceptUploader):
    ...
    print("\n💡 Next Steps:")
    print("   • View specific video: python scripts/visualize_concept_graph.py VIDEO_ID")
    print("   • Search concepts: python scripts/query_concepts.py --search 'topic'")
    print("   • Extract relationships: python scripts/extract_relationships.py VIDEO_ID")
