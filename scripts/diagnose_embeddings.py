...
def diagnose_video(video_id: str, sample_size: int = 20):
    ...
    try:
        ...
        print(
            f"   • Segments with embeddings: {with_embeddings}/{len(segments)} "
        ...
        for i in range(min(sample_size, len(segments) - 1)):
            if segments[i]["embedding"] and segments[i + 1]["embedding"]:
                ...
                if i < 5:
                    ...
                    print(
                        f"      Time gap: {segments[i+1]['start'] - segments[i]['end']:.1f}s"
                    )
        ...
        if similarities:
            ...
            for min_val, max_val, label in bins:
                ...
                bar = "█" * int(pct / 2)
                print(
                    f"   {min_val:.2f}-{max_val:.2f} ({label:9}): {bar} {pct:5.1f}% ({count})"
                )
            ...
            if mean_sim < 0.4:
                ...
                print(
                    f"      → Use LOWER thresholds (e.g., adjacent_threshold=0.40-0.50)"
                )
        ...
        for idx in [0, len(segments) // 2, len(segments) - 1]:
            ...
            if neighbor_sims:
                print(
                    f"      Top-8 neighbor similarities: {[f'{s:.3f}' for s in neighbor_sims[:8]]}"
                )
                print(
                    f"      Mean: {np.mean(neighbor_sims):.3f}, Min: {min(neighbor_sims):.3f}"
                )
            print()
