...
def print_detailed_groups(groups_data: Dict[str, Any], max_groups: int = 10) -> None:
    ...
    for i, group in enumerate(groups[:max_groups]):
        ...
        print(
            f"│ 🕐 Time:     {start_min:.1f}m → {end_min:.1f}m  (duration: {duration_min:.1f}m)"
        )
...
def print_boundary_analysis(groups_data: Dict[str, Any]) -> None:
    ...
    for i in range(len(groups) - 1):
        ...
        print(
            f"  Group {curr['group_id']:>2} → {next_group['group_id']:>2}:  "
...
def print_outlier_groups(groups_data: Dict[str, Any]) -> None:
    ...
    for group in groups:
        z_score = (
            (group["total_words"] - mean_words) / std_words if std_words > 0 else 0
        )
        if abs(z_score) > 1.5:
            found_word_outliers = True
            marker = "📈" if z_score > 0 else "📉"
            print(
                f"  {marker} Group {group['group_id']:>2}: {group['total_words']:>4} words "
                f"(z={z_score:+.2f})"
            )
    ...
    for group in groups:
        if group["avg_cohesion"] < mean_cohesion - 0.15:
            ...
            print(
                f"  ⚠️  Group {group['group_id']:>2}: cohesion={group['avg_cohesion']:.3f} "
...
def generate_timeline_view(groups_data: Dict[str, Any], width: int = 70) -> None:
    ...
    for group in groups:
        ...
        print(
            f"│{bar}│ {indicator} G{group['group_id']:>2} ({group['total_words']:>3}w)"
        )
